...
1. `install.packages`               needed infrequently from the internet
...
In the code below try commenting out `#tm_symbols()` and uncommenting `tm_symbols(col = "blue")`. You can also use `col=[column_name]` to set the colour of each point according to the data value stored in that column.
...
Next trying the highway network (lines).
...
Thirdly mapping the countries (polygons).
...
#tm_shape(africountries) + tm_polygons("pop_est")
#tm_shape(africountries) + tm_polygons("income_grp")

```

Fourthly, mapping gridded (raster) data that can represent e.g. remotely sensed or modelled data.

This uses `tm_raster` and specifies the `breaks` or cutoffs to the different colour bands. In this example if you use the default breaks by not specifying any arguments with `tm_raster()` (try it below) the map looks entirely one colour.  
...
Experiment with commenting out lines in the code below.
...
The maps created so far have been static, there are also great options for creating interactive maps, for web pages or reports, where readers can zoom, pan and enable/disable layers.
...
```{r tmap-interactive, exercise = TRUE, exercise.eval = TRUE, message = FALSE, warning = FALSE}
...
So far we have been using data that already exist within R as an R object.

The same things can be done on data coming from a file.

Spatial data can be read into R using the `sf::st_read()` for vector data (points, lines and polygons) and the `raster` package for gridded data.
...
library(sf)
filename1 <- system.file("extdata/africountries.shp", package="afrilearndata")
myobject1 <- sf::st_read(filename1)

library(raster)
filename2 <- system.file("extdata/afripop2020.tif", package="afrilearndata")
...
quiz(
  ...
  question("Which R packages can be used to make maps ?",
    ...
    incorrect = "All of these packages can be used to make maps ! Sometimes which you choose can be down to preference."
  )
)
```
