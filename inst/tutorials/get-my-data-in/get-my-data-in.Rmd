...
TODO add package dependencies visibly.
...
# 1. read from a csv file stored in the afrilearndata package as example to work with
...
## *.kml, geopackage *.gpkg & geojson *.json

For other spatial vector formats (e.g. kml, gpkg & geojson) the same approach as for a shapefile usually works i.e. `sf::read_sf(filename)`.
